...
export class GameService {
    public areaColumns = 10;
    public areaRows = 10;
    public area: Cell[][];
    public bombsCount = 25;
    public bombsModeCounter = 1;
    randomNumbers = [];
        ...
        try {
            if ((this.areaColumns > 1 && this.areaRows > 1) && (this.areaColumns <= 10 && this.areaRows <= 10)) {
                if (this.bombsCount <= (this.areaRows * this.areaColumns) - this.cellsToCheck) {
                    ...
                    throw({ message: 'Too much bombs for your gameboard' });
                }
            } else {
                throw({ message: 'Gameboard should be at least 2x2 and at most 10x10' });
            }
          } catch (err) {
    ...
    private fillWithBombs(rand1: number, rand2: number) {
